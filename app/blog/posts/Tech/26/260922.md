...
- The debugger and editor lack many features I consider mandatory: watch variable, "hover-to-see-value", easy way to see which file references a variable, debug variable being sorted by name. Also, unlike VS Code, you have to set up the debugger by reading docs, while VS Code, in a lot of cases, has debug config scaffolding so you can get started quickly. Another annoyance is that when debugging PHP, I have to launch the Xdebug server and the PHP server separately, instead of pressing "F5" once.
